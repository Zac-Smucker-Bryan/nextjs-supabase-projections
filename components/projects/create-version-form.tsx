...
export function CreateVersionForm({ projectId }: { projectId: string }) {
  ...
  return <form className="grid gap-3" onSubmit={(event) => {
    ...
    startTransition(async () => { const result = await createProjectionVersion(projectId, new FormData(form)); if (result.error) return setError(result.error); setError(null); form.reset(); close(); router.refresh(); });
  }}>
